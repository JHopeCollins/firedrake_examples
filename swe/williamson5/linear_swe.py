...
# time discretisation
delta_t =7.5*units.minute  # implicit timestep size
tmax = 5*earth.day         # simulation time
theta = 0.0                # implicit theta-method ( 0: backward Euler, 0.5: trapezium )
out_freq = 16              # number of delta_ts between snapshots
...
H = fd.Constant(case5.h0)    # mean depth
Omega = fd.Constant(earth.omega)
u0 = fd.Constant(case5.u0)    # max velocity
...
f = case5.coriolis_function(      *coords, Vf )
uexact = case5.velocity_function( *coords, V1 )
hexact = case5.depth_function(    *coords, V2 )
